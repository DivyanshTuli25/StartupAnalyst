...
if selected == "Home":
    ...
    if submit1:
        if pdf_document is not None and url_input:
            st.subheader("The response is being processed...")
    ...
    if submit2:
        if "first_result" in st.session_state:
            ...
            st.markdown(f"## Here is the Final Result \n\n {final}")
        else:
            st.error("Please run the first analysis to get the initial results.")
...
st.markdown(
    ...
    © 2024 Startup Analyst. All rights reserved.
    </footer>
